...
export class GenerateCodeCellCommand extends BaseAICommand {
    ...
    private buildNotebookContext(): NotebookContext {
        ...
        const context: NotebookContext = {
            ...
            notebookModel: model, // Include the full notebook model for complete context
            notebookType: model?.cells?.length > 0 ? 'existing' : 'new'
        };
        ...
        if (reactiveSystem && reactiveSystem.reactiveStore) {
            try {
                ...
                // Filter and ensure we have strings, then deduplicate
                const stringVariables = allVariableNames.filter((name: any): name is string => typeof name === 'string');
                context.variables = Array.from(new Set(stringVariables)) as string[];
                ...
                // Also log variable values for debugging
                const variableValues: Record<string, any> = {};
                allVariableNames.forEach((varName: any) => {
                    if (typeof varName === 'string') {
                        try {
                            variableValues[varName] = reactiveSystem.reactiveStore.getValue(varName);
                        } catch (error) {
                            log.debug(`[${contextId}] Could not get value for variable ${varName}:`, error);
                        }
                    }
                });

                log.debug(`[${contextId}] Variable values`, variableValues);
            } catch (error) {
...
export class GenerateCellCommand extends BaseAICommand {
    ...
    private buildNotebookContext(): NotebookContext {
        ...
        log.debug(`[${contextId}] Building notebook context (GenerateNotebook)`, {
            hasModel: !!model,
            cellCount: model?.cells?.length || 0
        });
        ...
        model.cells.forEach((cell: CellDefinition, index: number) => {
            ...
            switch (cell.type) {
                ...
                case 'formula':
                    const formulaCell = cell as FormulaCellDefinition;
                    context.variables.push(formulaCell.variableName);
                    context.cellContents.push({
                        type: 'formula',
                ...
                case 'input':
                    const inputCell = cell as InputCellDefinition;
                    context.variables.push(inputCell.variableName);
                    context.cellContents.push({
                        type: 'input',
        ...
        context.variables = [...new Set(context.variables)];
    ...
    private async createCellFromDefinition(cellDefinition: CellDefinition): Promise<void> {
        ...
        try {
            ...
            // Ensure the cell has a unique ID
            const finalCellDefinition = {
                ...cellDefinition,
                id: `cell_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
            };

            log.debug(`[${createId}] Inserting AI-generated cell directly into model`, {
                cellType: finalCellDefinition.type,
                cellId: finalCellDefinition.id,
                insertIndex,
                cellDefinition: finalCellDefinition
            });

            // Insert the AI-generated cell directly into the model
            const newCells = [...model.cells];
            newCells.splice(insertIndex, 0, finalCellDefinition);
            ...
            log.info(`[${createId}] AI-generated cell inserted successfully`, {
                cellType: finalCellDefinition.type,
                cellId: finalCellDefinition.id,
                insertIndex
            });
